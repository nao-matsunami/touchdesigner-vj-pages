...
const repo = "nao-matsunami/touchdesigner-vj-pages";
...
function run(command, args, cwd, stdio = "inherit") {
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => { if (code === 0) resolve(); else reject(new Error(`${command} ${args.join(" ")} failed with code ${code}`)); });
  });
}
function runCapture(command, args, cwd) {
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => { if (code === 0) resolve(stdout.trim()); else reject(new Error(stderr.trim() || `${command} ${args.join(" ")} failed with code ${code}`)); });
  });
}
...
async function main() {
  ...
  await run("git", ["commit", "-m", `Publish TouchDesigner VJ site ${stamp}`], targetDir);
